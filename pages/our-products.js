import Image from 'next/image';
import seedlings from '../public/seedlings.jpg'
import { MdOutlineCompost, MdOutlineContactSupport } from "react-icons/md";
import { GiChemicalDrop,GiWorld, GiPlantSeed, GiFarmTractor, GiChemicalTank, GiFarmer, GiMasterOfArms, GiSeedling } from "react-icons/gi";




export default function OurProducts(){
    return(
      <>
       <div className="left-0 top-0 w-full">

<div className="header-bg1 bg-cover bg-no-repeat bg-center bg-[#08011DA8] bg-blend-darken relative">


<div className=" pl-6 pr-8 pb-6 relative flex lg:justify-center lg:text-center">
    <div className="static lg:basis-2/3 md:basis-3/4 md:mt-5">
    <h1 className="text-3xl lg:text-7xl md:leading-[70px] 
    pt-[15vh] md:text-4xl lg:leading-[100px] leading-[50px] 
    font-extrabold font-Poppins gradient__tnext text-white/90 
    content-center">Our Products</h1>
    <button className="pl-2">
    <div className="border-b-white/60 border-b-[6px] w-[65px] mb-5 mt-6">

</div>
</button>
    
    </div>
    </div>
    </div>



<div className="pb-10 md:pt-0 bg-[#334b35]">
    <div className="pt-12 justify-center text-center">
    
    <h1 className="text-[#b3c5b5] font-[400] text-lg">
                    Our Products Achive
                </h1>
           
                <h1 className="text-white font-bold mb-1 text-[30px] px-3 lg:text-[40px] lg:mb-5 pb-1">
                    AgroChemicals
                </h1>
                <button>
                <div className="border-b-[#b3c5b5] border-b-[5px] w-[65px] mb-4">

</div>

</button>
                
  
    </div>
            <div className="font-light px-4 md:container md:mx-auto md:lg-10 grid lg:grid-cols-3 grid-cols-1 gap-y-[17px] 2xl:gap-3 justify-center">
                
                
              
                <div className="bg-[#263c28] mt-16 md:mx-24 lg:ml-10 text-white/90 text-center rounded-[15px] shadow-sm shaddow-[#b3c5b5]">
                <div className="justify-center content-center text-center">
                  
                  
              
                  
                  <button><div
            style={{ size:'', color:'#f1cf69' }}
            className=''
          >
             <div className="bg-[#263c28] w-[150px] h-[150px] shadow-md border-gray-400 p-2 relative rounded-full -mt-16">
             <Image
                    src={seedlings}
                    width='200px'
                    height='200px'
                    alt='/'
                    className='rounded-full'
                  />
            </div>
            
                </div>
                </button>
                </div>
                <div className="lg:ml-5 pb-5 px-8 ">
                

                    <h3 className="font-extrabold text-2xl py-4">Abacus Advance</h3>
                    <p className="leading-8 font-light">
                    <b>Ingredients:</b> Pyraclostrobin 62.5+ Epoxiconazole 62.5<br/>
                    <b>Used On: </b> Wheat/Soyabean/Maize
                    </p>

                   
                   

                   
                    </div>
                    </div>
                    {/*  */}

                    <div className="bg-[#263c28] mt-16 md:mx-24 lg:ml-10 text-white/90 text-center rounded-[15px] shadow-sm shaddow-[#b3c5b5]">
                <div className="justify-center content-center text-center">
                  
                  
              
                  
                  <button><div
            style={{ size:'', color:'#f1cf69' }}
            className=''
          >
             <div className="bg-[#263c28] w-[150px] h-[150px] shadow-md border-gray-400 p-2 relative rounded-full -mt-16">
             <Image
                    src={seedlings}
                    width='200px'
                    height='200px'
                    alt='/'
                    className='rounded-full'
                  />
            </div>
            
                </div>
                </button>
                </div>
                <div className="lg:ml-5 pb-5 px-8 ">
                

                    <h3 className="font-extrabold text-2xl py-4">Abacus Advance</h3>
                    <p className="leading-8 font-light">
                    <b>Ingredients:</b> Pyraclostrobin 62.5+ Epoxiconazole 62.5<br/>
                    <b>Used On: </b> Wheat/Soyabean/Maize
                    </p>

                   
                   

                   
                    </div>
                    </div>
                    
                    {/*  */}
              
           
                    <div className="bg-[#263c28] mt-16 md:mx-24 lg:ml-10 text-white/90 text-center rounded-[15px] shadow-sm shaddow-[#b3c5b5]">
                <div className="justify-center content-center text-center">
                  
                  
              
                  
                  <button><div
            style={{ size:'', color:'#f1cf69' }}
            className=''
          >
             <div className="bg-[#263c28] w-[150px] h-[150px] shadow-md border-gray-400 p-2 relative rounded-full -mt-16">
             <Image
                    src={seedlings}
                    width='200px'
                    height='200px'
                    alt='/'
                    className='rounded-full'
                  />
            </div>
            
                </div>
                </button>
                </div>
                <div className="lg:ml-5 pb-5 px-8 ">
                

                    <h3 className="font-extrabold text-2xl py-4">Abacus Advance</h3>
                    <p className="leading-8 font-light">
                    <b>Ingredients:</b> Pyraclostrobin 62.5+ Epoxiconazole 62.5<br/>
                    <b>Used On: </b> Wheat/Soyabean/Maize
                    </p>

                   
                   

                   
                    </div>
                    </div>
                    {/*  */}

                    
                    <div className="bg-[#263c28] mt-16 md:mx-24 lg:ml-10 text-white/90 text-center rounded-[15px] shadow-sm shaddow-[#b3c5b5]">
                <div className="justify-center content-center text-center">
                  
                  
              
                  
                  <button><div
            style={{ size:'', color:'#f1cf69' }}
            className=''
          >
             <div className="bg-[#263c28] w-[150px] h-[150px] shadow-md border-gray-400 p-2 relative rounded-full -mt-16">
             <Image
                    src={seedlings}
                    width='200px'
                    height='200px'
                    alt='/'
                    className='rounded-full'
                  />
            </div>
            
                </div>
                </button>
                </div>
                <div className="lg:ml-5 pb-5 px-8 ">
                

                    <h3 className="font-extrabold text-2xl py-4">Abacus Advance</h3>
                    <p className="leading-8 font-light">
                    <b>Ingredients:</b> Pyraclostrobin 62.5+ Epoxiconazole 62.5<br/>
                    <b>Used On: </b> Wheat/Soyabean/Maize
                    </p>

                   
                   

                   
                    </div>
                    </div>
                    {/*  */}

                        </div>
                        </div>


    
    
  </div>


</>
    )
}